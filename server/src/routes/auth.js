...
router.post("/github", async (req, res) => {
  try {
    ...
    console.log(`🏗️ OAuth: Client Secret: ${process.env.GITHUB_CLIENT_SECRET ? 'PRESENT' : 'MISSING'}`);
    ...
    console.log(`✅ OAuth: GitHub responded with status ${tokenResponse.status}`);
    console.log(`🔍 OAuth: Response data:`, tokenResponse.data);

    const accessToken = tokenResponse.data.access_token;    if (!accessToken) {
      console.log(`❌ OAuth: No access token received. Full response:`, tokenResponse.data);
      ...
      if (tokenResponse.data.error) {
        switch (tokenResponse.data.error) {
          case 'bad_verification_code':
            userMessage = "Authorization code is invalid or expired. Please try logging in again.";
            console.log(`🔍 OAuth: GitHub rejected the authorization code - likely expired or already used`);
            break;
          ...
          case 'redirect_uri_mismatch':
            userMessage = "OAuth redirect URI mismatch. Please contact support.";
            console.log(`🚨 OAuth: Redirect URI not configured in GitHub OAuth app`);
            break;
          default:
            userMessage = `GitHub authentication error: ${tokenResponse.data.error_description || tokenResponse.data.error}`;
        }
    ...
    console.log(`🔑 OAuth: Using access token: ${accessToken.substring(0, 10)}...`);
    let userResponse;
    let emailResponse;
    try {
      ...
      if (emailResponse.data?.length > 0) {
        console.log(`📧 OAuth: Available emails:`, emailResponse.data.map(e => ({ email: e.email, primary: e.primary, verified: e.verified })));
      }} catch (apiError) {
      console.log(`❌ OAuth: GitHub API call failed with status ${apiError.response?.status}`);
      ...
      if (apiError.config?.url?.includes('/user/emails')) {
        console.log(`⚠️ OAuth: Emails API failed, will continue without email data`);
        emailResponse = { data: [] }; // Set empty array so flow continues
      } else if (apiError.response?.status === 401) {
        return res.status(400).json({
          success: false,
          message: "Failed to validate access token with GitHub. Please try logging in again.",
          debug: process.env.NODE_ENV === "development" ? {
            error: "GitHub API returned 401 - access token may be invalid",
    ...
    if (!userEmail && emailResponse?.data?.length > 0) {
      ...
      userEmail = primaryEmail?.email || verifiedEmail?.email || emailResponse.data[0].email;
    ...
    if (!user) {
      ...
      console.log(`🆕 OAuth: Created new user: ${user.username} (${user.email})`);
      ...
      console.log(`🔄 OAuth: Updated existing user: ${user.username} (${user.email})`);
    }    // Generate JWT token
