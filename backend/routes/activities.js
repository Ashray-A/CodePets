...
router.get("/today", authenticateToken, async (req, res) => {
  try {
    ...
    const stats = {
      commits: todayActivities.filter((a) => a.type === "commit").length,
      codingTime: todayActivities
        .filter((a) => a.type === "coding_session")
        .reduce((sum, a) => sum + (a.data.duration || 0), 0),
      totalXP: todayActivities.reduce((sum, a) => sum + a.experience, 0),
    ...
    res.status(500).json({ message: "Failed to get today's activity stats" });
  }
});
